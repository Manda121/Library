...
CREATE TABLE status_prolongement(
   id_status_prolongement INT AUTO_INCREMENT,
   status_prolongement VARCHAR(50),
   PRIMARY KEY(id_status_prolongement)
);
...
CREATE TABLE prolongement(
   ...
   description TEXT,
   id_pret INT NOT NULL,
   id_status_prolongement INT DEFAULT 0,
   PRIMARY KEY(id_prolongement),
   FOREIGN KEY(id_pret) REFERENCES pret(id_pret),
   FOREIGN KEY(id_status_prolongement) REFERENCES status_prolongement(id_status_prolongement)
);
