...
public class UserService {
   ...
   public String updateAllAdherentStatuses() {
      ...
      while(var6.hasNext()) {
         ...
         if (currentStatutId != 2) {
            ...
            boolean allPenaltiesExpired = penalties.isEmpty() || penalties.stream().allMatch((p) -> {
               return today.isAfter(p.getDateRemisePenalisee().plusDays((long)adherent.getTypeProfil().getNbJourPenalite()));
            });
      ...
      String message = statusChanged ? "Mise à jour des statuts terminée. " + activeCount + " adhérents actifs, " + inactiveCount + " adhérents inactifs." : "Aucun changement de statut requis.";
      return message;
   }

   @Transactional
   public String processLoan(int exemplaireId, int adherentId, LocalDate dateRemise) {
      ...
      if (adherent != null && adherent.getStatut().getIdStatut() == 1) {
               ...
               if (exemplaire == null) {
                  ...
                  if (nblivres <= nbreservation + nbpret) {
                     ...
                     if (exemplaire.getAgeMinimal() != 0 && adherent.getDateNaissance() != null) {
                        try {
                           ...
                           if (ageAdherent < exemplaire.getAgeMinimal()) {
                              ...
                              return "L'adhérent n'a pas l'âge minimum requis (" + var10000 + " ans) pour emprunter cet exemplaire. Il(elle) a actuellement " + ageAdherent;
                           }
                     ...
                     if (hasSameExemplaire) {
                        ...
                        EtatReservation canceledState = (EtatReservation)this.etatReservationRepository.findById(3).orElseThrow(() -> {
                        ...
                        for(holidayMessage = ""; this.jourFerierRepository.existsByDateJourFerier(dateRemise); holidayMessage = "La date de remise a été ajustée au " + dateRemise + " car le jour initial était férié.") {
                        ...
                        TypeLecture defaultTypeLecture = (TypeLecture)this.typeLectureRepository.findById(1).orElseThrow(() -> {
         ...
         return adherent.getStatut().getIdStatut() == 2 ? "L aderent est suspendu" : "L'adhérent n'est pas actif ou n'existe pas.";
      }
   }
   ...
   public String processReturn(int pretId) {
      ...
      if (pret == null) {
         ...
         if (isOverdue) {
            ...
            Statut suspendedStatus = (Statut)this.statutRepository.findById(2).orElseThrow(() -> {
